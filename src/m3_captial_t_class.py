...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        h_rect_point_start = (rg.Point(self.intersection_center.x-(
            self.width/2),self.intersection_center.y -
                                       self.letter_thickness/2))
        h_rect_point_end = (rg.Point(self.intersection_center.x+(
            self.width/2),self.intersection_center.y +
                                       self.letter_thickness/2))

        h_rect = rg.Rectangle(h_rect_point_start, h_rect_point_end)

        v_rect_point_start = (rg.Point(self.intersection_center.x-(
            self.letter_thickness/2),self.intersection_center.y -
                                       self.letter_thickness/2))
        v_rect_point_end = (rg.Point(self.intersection_center.x+(
            self.letter_thickness/2),self.intersection_center.y +
    ...
    def clone(self):
        ...
        new.set_colors(self.h_rect.fill_color,self.h_rect.outline_color)
        return (new)
# ----------------------------------------------------------------------
